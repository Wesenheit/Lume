use crate::core::{Matrix,Renderable,Region,Structure};
use crate::cpu::Cpu;
...
impl Renderable for CombinedPattern {
    ...
    fn render(&mut self, matrix: &mut Matrix) {
        for (pattern,region) in self.patterns.iter_mut().zip(self.ranges.iter()){
            pattern.render_region(&mut matrix.rows[region.lower..region.upper], Option::Some(region));
        }
    }
}
...
impl CombinedPattern {
    pub fn from_yaml(path: &str) -> Self {
        ...
        let(ranges,patterns):(Vec<Region>,Vec<Box<dyn Renderable>>) = config.modules.into_iter().map(|entry| {
            let region = Region{lower:entry.region.start,upper:entry.region.end};
            let pattern: Box<dyn Renderable> = match entry.name.as_str() {
                "cpu"    => Box::new(Cpu::from_config(entry.config,config.step)),
                "random" => Box::new(CM5{sliding:config.step}),
        ...
        return Self { patterns, ranges, sliding:config.step ,size:max.expect("no ranges specified!")}
    }
